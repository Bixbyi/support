module.exports = {
	...
	execute(client, message, args, prefix, knex) {
        const amount = args // 적은 값 불러오기
        var isNum = !isNaN(amount) // 숫자인지 아닌지
        if (!message.member.hasPermission(['MANAGE_MESSAGES'])) return message.channel.send('저의 권한이 너무 낮습니다!') // 유저 권한 확인
            if (!amount||isNum && (amount <= 0.9 || 99.9 < amount) || isNum == false) { // 숫자가 1~99가 아니거나 숫자가 아닐시
              message.channel.send(`${message.member}, 1 ~ 99 사이의 숫자를 입력해주세요!`)
              return
            } else {
              message.channel
              .bulkDelete(parseInt(amount) + 1, true) // 메시지 삭제
              // .then(() => {
